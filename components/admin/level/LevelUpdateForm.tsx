...
const LevelUpdateForm = (props: LevelUpdateFormProps) => {
	...
	const formValidate = (): boolean => {
		...
		if (levelData.title.length < 1) {
			validateTitle = false;
			setTitleError(errors.field.empty);
		}

		if (levelData.title.length > 0) {
			validateTitle = true;
			setTitleError(null);
		}
	...
	const onChange = (
		...
		setLevelData({ ...levelData, [e.target.name]: e.target.value.trim() });
	};
...
export default LevelUpdateForm;
